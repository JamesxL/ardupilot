...
{
    struct log_AIRSPD pkt = {
        LOG_PACKET_HEADER_INIT(LOG_AIRSPD_MSG),
        airspeed_cm         : (int16_t)airspeed.get_airspeed_cm()
    };
    DataFlash.WriteBlock(&pkt, sizeof(pkt));
}
...
//write a RCinput packet. ch1-4 and mode channel
struct PACKED log_RCI {
    LOG_PACKET_HEADER;
    int16_t throttle_in;
    int16_t pitch_in;
    int16_t roll_in;
...
{
    struct log_RCI pkt = {
        LOG_PACKET_HEADER_INIT(LOG_RCI_MSG),
        throttle_in             : channel_throttle->control_in,
        pitch_in                : channel_pitch->control_in,
        roll_in                 : channel_roll->control_in,
        rudder_in               : channel_rudder->control_in,
    };
    DataFlash.WriteBlock(&pkt, sizeof(pkt));
}
...
static const struct LogStructure log_structure[] PROGMEM = {
    ...
    { LOG_RCO_MSG, sizeof(log_RCO),             
      "RCO", "hhhh",        "ThrOut,PitOut,RolOut,RudOut" },
